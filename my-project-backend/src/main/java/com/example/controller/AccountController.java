...
public class AccountController {
    ...
    public RestBean<Void> saveDetails(@RequestAttribute(Const.ATTR_USER_ID) int id, @RequestBody @Valid DetailsSaveVO vo) {
        ...
        return success ? RestBean.success() : RestBean.failure(400, "用户名已被占用！");
    }

    @PostMapping("/modify-email")
    public RestBean<Void> modifyEmail(@RequestAttribute(Const.ATTR_USER_ID) int id, @RequestBody @Valid ModifyEmailVO vo) {

        return controllerUtils.messageHandle(() -> service.modifyEmail(id, vo));
    }
    ...
    public RestBean<Void> changePassword(@RequestAttribute(Const.ATTR_USER_ID) int id, @RequestBody @Valid ChangePasswordVO vo) {
        return controllerUtils.messageHandle(() -> service.changePassword(id, vo));
    }
