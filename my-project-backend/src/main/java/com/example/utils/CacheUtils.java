...
public class CacheUtils {
    ...
    public  <T> List<T> takeListFromCache(String key , Class<T> itemType){  // 取数据
        String s  = template.opsForValue().get(key);
        if ( s == null ) return  null;
        return  JSONArray.parseArray(s).toList(itemType);
    }
    public <T> void  saveToCache(String key , T data , long expire){ // 存数据
        template.opsForValue().set(key, JSONObject.from(data).toJSONString(),expire, TimeUnit.SECONDS);
    }
    public <T> void  saveListToCache(String key , List<T> list , long expire){ // 存数据
    ...
    public  void deleteCacheParttern(String key){
